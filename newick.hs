...
splitOn ::  [Char] ->[Char] -> [[Char]] --- I may need to get rid off this fuction (is not used)
...
openParent :: [Char] -> [Int] -- Takes a newick string and returns a tuple of two list with the indexes of opening and closing parentheses.
openParent xs = '(' `List.elemIndices` xs


closeParent :: [Char] ->  [Int] -- Takes a newick string and returns a list with the indexes of closing parentheses.
...
-- assymDiff :: [String] -> [String] -> [Bool] -- where the list of input strings are list of bipartitions
-- assymDiff [] (ys)  =  []
-- assymDiff (x:xs) ys = foldl (\x => if issameSplit x then ) False ys: assymDiff xs ys
...
getSupports :: String -> [Float]
getSupports tree 
  | sub == "" = []
  | support == ";" = 100.0 : getSupports (tail sub) 
  | support == "" = 0 :  getSupports (tail sub) 
  | otherwise = read support : getSupports (tail sub) 
  where
    sub =  dropWhile (/=')') tree
    support =  tail $ takeWhile (/= ':') sub
